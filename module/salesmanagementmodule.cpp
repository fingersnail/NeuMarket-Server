...
QByteArray SalesManagementModule::handleRequest(int methodName, const QString &parameter1, const QString &parameter2, const QString &parameter3, const QString &parameter4, const QString &parameter5, const QString &parameter6, const QString &parameter7)
{
    QByteArray sendMsg;
    bool excuteResult;
    ...
    case QueryAllSales_Method:
        resultVector = queryAllSales();
        variantList = Utilities::vector2VariantList(resultVector);
        break;
    case QuerySalesById_Method:
        resultVector = querySalesById(parameter1.toInt());
        variantList = Utilities::vector2VariantList(resultVector);
        break;
    case QuerySalesByName_Method:
        resultVector = querySalesByName(parameter1);
        variantList = Utilities::vector2VariantList(resultVector);
        break;
    case AddSales_Method:
        excuteResult = addSales(parameter1.toInt(), parameter2.toInt(), parameter3.toDouble());
        variantList = Utilities::bool2VariantList(excuteResult);
        break;
    case DeleteSales_Method:
        excuteResult = deleteSales(parameter1.toInt());
        variantList = Utilities::bool2VariantList(excuteResult);
        break;
    case QuerySalesKindStatistic_Method:
        resultVector = querySalesKindStatistic(QDate::fromString(parameter1, "yyyy-MM-dd"), QDate::fromString(parameter2, "yyyy-MM-dd"));
        variantList = Utilities::vector2VariantList(resultVector);
        break;
...
QVector<shared_ptr<AbstractObject> > SalesManagementModule::queryAllSales()
{
    QVector<shared_ptr<AbstractObject> > sales;
    QString statement = "SELECT * FROM SalesRecord";
...
void SalesManagementModule::queryResult2SalesVector(QSqlQuery &query, QVector<shared_ptr<AbstractObject> > &vec)
{
    ...
    while (query.next()) {
        ...
        temp.setMoneyAmount(query.value(3).toDouble());
        temp.setTime(query.value(4).toDateTime());
